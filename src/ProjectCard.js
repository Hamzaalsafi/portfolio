...
export function ProjectCard({ project }) {
    ...
    >
            ...
            <div className='md:w-1/2 md:pr-10 justify-start'>
                <h1 onClick={handleLinkClickLink} className='text-2xl cursor-pointer font-bold'>{project.title || " "}</h1>
                <h2 className='text-lg pt-1 font-bold'>{project.title2 || " "}</h2>
                <p className={`text-[1.17rem] ${mode !== "dark" ? "text-zinc-800" : "text-zinc-300"}`}>{project.description || " "}</p>
                ...
                <ul className={`list-disc text-[1.1rem] pl-5 ${mode !== "dark" ? "text-zinc-800" : "text-zinc-300"}`}>
                    {project.keyFeatures && project.keyFeatures.map((li, index) => (
                        <li key={index}>{li}</li>
                    ))}
                </ul>
                <div className='flex gap-2.5 pt-2 items-center flex-wrap'>
                    {project.tooTip && project.tooTip.map((title, index) => (
                        <Tooltip
                            key={index}
                            PopperProps={{
                                modifiers: [
                ...
                <div className='md:absolute relative z-50 text-slate-950 text-3xl gap-5 text-center mb-2 w-full flex justify-center items-center'>
                    ...
                    <motion.img
                        ...
                        animate={{
                            ...
                            backgroundColor: showOptions === "laptop" ? "rgba(255, 255, 255, 1)" : "rgba(0, 0, 0, 0)",
                        }}
                        transition={{
                            duration: 0.2,
                            ease: "easeOut",
